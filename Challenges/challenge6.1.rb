class EventManager
  ...
  def findEventsNear(lat, lon, maxDist=5000)
    ...
    @eventList.each { |event|
      ...
      if @calc.distance(lat, lon, elat, elon) < maxDist
        events << event
      end
    }
  ...
  def findEarthquakesNear(cityName)
    ...
    @eventList.each { |event|
      ...
      if type == "earthquake"
        if @calc.distance(elat, elon, clat, clon) < 5000
          events << event
        end
      end
    }
...
em.addEvent(Event.new("my favorite hotspring", "hotspring", 46.1231, -115.23234))
em.addEvent(Event.new("Cityton", "city", 40.5232, -121.23453))
em.addEvent(Event.new("yet another volcano", "volcano", 15.52232, -107.03521, 256923))
